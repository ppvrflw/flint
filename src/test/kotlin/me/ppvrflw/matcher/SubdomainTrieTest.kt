    ...
    FunSpec({
      ...
      context("simple tests") {
        withTests(
          ...
          val trie = SubdomainTrie<String>().apply { insert(parts, value) }
      ...
      test("two values on the same subdomain") {
        val trie =
            SubdomainTrie<String>().apply {
              insert(listOf("sub"), "sub.api.example.com - phishing")
              insert(listOf("sub"), "sub.api.example.com - spam")
            }

        trie.match(listOf("sub")) shouldBe
            listOf("sub.api.example.com - phishing", "sub.api.example.com - spam")
      }
      ...
      test("null input returns empty list") {
        val trie = SubdomainTrie<String>().apply { insert(listOf("sub"), "sub.example.com") }
