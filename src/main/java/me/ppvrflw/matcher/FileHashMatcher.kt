...
class FileHashMatcher<V> : Matcher<FileHashRecord, V> {
  private val fileHashMap: ConcurrentHashMap<String, CopyOnWriteArrayList<V>> = ConcurrentHashMap()

  override fun insert(key: FileHashRecord, value: V) {
    val values = fileHashMap.getOrPut(key.hash) { CopyOnWriteArrayList() }
    if (value !in values) {
      values.add(value)
    }
  }
